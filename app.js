...
function login(){
  ...
  protect();
  document.querySelector("#togglePassword")?.addEventListener("click",()=>{const p=document.querySelector("#password"),b=document.querySelector("#togglePassword");if(!p||!b)return;const show=p.type==="password";p.type=show?"text":"password";b.textContent=show?"🙈":"👁";b.setAttribute("aria-label",show?"Hide password":"Show password");});
  const go=async()=>{
    const msg=document.querySelector("#msg");
    try{
      msg.textContent="Checking authorization…";msg.className="form-message loading";
      const list=await fetch("https://portal.iarco.org/data/users.json",{cache:"no-store"}).then(r=>{if(!r.ok)throw Error("Could not load participant data");return r.json()});
